...
class FbController extends base_controller_1.BaseController {
    ...
    getImageWebView() {
        ...
        console.log("**********************");
        this.response.body = { psid: this.request.queryParams["psid"] };
        this.response.filePath = app_config_1.AppConfig.imageTemplate;
    }
    uploadImage() {
        ...
        try {
            if (this.request.files) {
                ...
                console.log();
                var filename = uuid_1.v4() + path.extname(avatar.name);
                ...
                mkdirp('./../../QuickA/QuickA.Api/Files/Photo/' + fbId, function (err) {
                    ...
                    console.log("\n fbId=" + fbId);
                    let sessionService = new session_service_1.SessionService();
                    new send_response_chain_1.SendResponseChain(sessionService, fbId, app_config_1.AppConfig.fbAccessToken).executeProcess(filename, false, true);
                });
                this.response.status = 200;
